...
export default function SellNFT () {
    const [formParams, updateFormParams] = useState({ name: '', description: '', price: '' , noOfDays : ''});
    ...
    async function uploadMetadataToIPFS() {
        const {name, description, price,waranty} = formParams;
        
        if( !name || !description || !price || !fileURL || !waranty  )
            return;

        const nftJSON = {
            name, description, price,waranty, image: fileURL
        }
    ...
    async function listNFT(e) {
        ...
        try {
            ...
            updateMessage("");
            updateFormParams({ name: '', description: '', price: '' , waranty : ''});
    ...
    return (
        ...
        <div className="flex flex-col place-items-center mt-10" id="nftForm">
            <form className="bg-sky-100 shadow-md rounded px-8 pt-4 pb-8 mb-4">
            <h3 className="text-center font-bold text-purple-500 mb-8">Upload your NFT </h3>
                ...
                <div className="mb-6">
                    <label className="block text-purple-500 text-sm font-bold mb-2" htmlFor="price">waranty </label>
                    <input className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" type="number"  value={formParams.waranty} onChange={e => updateFormParams({...formParams, waranty: e.target.value})}></input>
                </div>
                <div>
